...
# `work_order`：运维执行域（一期工单）预留的保留子系统码 —— 其数据是**独立物理表**
# （不进 records，故本就不会进 total）；此处登记为排除项属**防御性**声明：
# 万一将来有人把工单域数据塞进 records 并挂到该子系统码下，total 也不会被污染（AC-08）。
_LEDGER_EXCLUDED_SUBSYSTEMS = ("elec_dwg", "room", "work_order")
