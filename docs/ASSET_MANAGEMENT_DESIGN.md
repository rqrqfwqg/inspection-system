...
| `BA系统设备清单_整理汇总.xlsx` | VRV空调（等 4 sheet） | 暖通 `hvac` | `ba_hvac` / 暖通BA点表（2026-09-15 四表合一，category 区分来源） | 设备编号★、来源类别、序号、组别 | 453 |
...
| 同上 | 一氧化碳检测（等 2 sheet） | 弱电 `weak` | `ba_gas_monitor` / 气体监测BA点表（2026-09-15 两表合一） | 设备编号★、来源类别、序号、组别、状态 | 40 |
...
| `C-B1-S-1DEMO-1_1#`（CO 检测，含 `#`） | found=True | 弱电系统 / 一氧化碳检测 + **BA问题清单**（聚合到设备） |
